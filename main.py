...
if sweep.get("enabled", False):
    ...
    if hp.get("agent_type") not in ("grpo", "grpo_batch"):
        raise ValueError("Sweep is supported only for GRPO training (including grpo_mc). Set 'agent_type' to 'grpo'.")
    ...
    divergences = sweep.get("divergences", [hp.get("divergence", "kl")])
    log_root = sweep.get("log_root", "logs/kl_sweep")
    run_tag = sweep.get("run_tag", "kl_sweep")

    total_runs = len(envs) * len(kl_coefs) * len(seeds) * len(grpo_mc_values) * len(divergences)
    current_run = 0
        ...
        for grpo_mc in grpo_mc_values:
            ...
            for kl_coef in kl_coefs:
                ...
                for divergence in divergences:
                    div_tag = _safe_name(str(divergence))
                    for seed in seeds:
                        current_run += 1
                        ...
                        run_hp["divergence"] = str(divergence).lower()
                        run_hp["run_id"] = f"{run_tag}__{env_safe}__{div_tag}__{mc_tag}__kl_{kl_tag}__seed_{seed}"
                        run_hp["log_dir"] = os.path.join(
                            log_root, env_safe, div_tag, mc_tag, f"kl_{kl_coef}", f"seed_{seed}"
                        )
                        ...
                        print(
                            ...
                            f"env={env_name} | divergence={divergence} | grpo_mc={mc_flag} | kl_coef={kl_coef} | seed={seed}"
                        )
